...
export default function UsersTable({ initialRows = [], onRowsChange, allRows = [] }) {
  ...
  return (
    <div className="w-full overflow-auto rounded-md border">
        ...
        <TableHeader>
          <TableRow>
            <TableHead className="w-[48px]">
              <Checkbox aria-label="Select all" checked={allChecked} onCheckedChange={(v) => toggleAll(!!v)} />
            </TableHead>
      ...
      <ApplicantDrawer
        ...
        applicant={rows.find((r) => r.id === openId) || null}
        onApplicantUpdated={(updated) => {
          if (!updated) return
          const merged = (allRows || []).map((r) => (r.id === updated.id ? updated : r))
          onRowsChange?.(merged)
        }}
      />
    </div>

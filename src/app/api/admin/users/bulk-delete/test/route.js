...
/**
 * Test endpoint for bulk delete functionality
...
export async function POST(request) {
  ...
  try {
    ...
    if (!sessionCookie) {
      logger.debug("API BULK DELETE TEST: No session cookie")
    ...
    const { testApplicantIds = [] } = await request.json()
    ...
    try {
      await base('Applicants').select({ maxRecords: 1 }).firstPage()
      connectionTest = true
    } catch (err) {
    ...
    if (testApplicantIds.length > 0) {
      for (const id of testApplicantIds) {
        try {
          await base('Applicants').find(id)
          existingIds.push(id)
        } catch (err) {
    ...
    const simulatedResults = {
      totalProcessed: testApplicantIds.length,
      wouldSucceed: existingIds.length,
      wouldFail: nonExistentIds.length,
      processingTime: Date.now()
    }

    logger.info(`Bulk delete test completed for user: ${userEmail}`)

    return Response.json({
      success: true,
      message: "Bulk delete test completed successfully",
      testResults: {
        connectionTest,
        simulatedResults,
        existingIds,
    ...
    logger.error("Error in bulk delete test:", error)
    return Response.json({ 
      error: "Test failed", 
